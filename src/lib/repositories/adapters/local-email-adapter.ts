...
import { loadJson, removeItem, saveJson } from '$lib/storage/local-storage';
...
export async function replaceEmails(nextEmails: EmailItem[]) {
	emails = nextEmails.map((email) => ({ ...email }));
	persist();

	return getEmailsSnapshot();
}

export async function resetEmailsToDefault() {
	emails = mockEmails.map((email) => ({ ...email }));
	persist();
...
export async function clearStoredEmails() {
