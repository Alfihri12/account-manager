...
export async function replaceAccounts(nextAccounts: AccountItem[]) {
	accounts = cloneAccounts(nextAccounts);
	persist();

	return getAccountsSnapshot();
}

export async function resetAccountsToDefault() {
	accounts = cloneAccounts(mockAccounts);
	persist();
